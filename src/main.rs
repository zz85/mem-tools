...
const PAGE_FLAGS: &[(u64, &str, &str, FlagCategory)] = &[
    ...
    (1 << 2,  "REFERENCED",  "Page has been referenced", FlagCategory::Usage),
    (1 << 3,  "UPTODATE",    "Page is up to date", FlagCategory::State),
    (1 << 4,  "DIRTY",       "Page is dirty", FlagCategory::State),
    (1 << 5,  "LRU",         "Page is on LRU list", FlagCategory::Memory),
    (1 << 6,  "ACTIVE",      "Page is on active list", FlagCategory::Memory),
    (1 << 7,  "SLAB",        "Page is slab allocated", FlagCategory::Allocation),
    (1 << 8,  "WRITEBACK",   "Page is under writeback", FlagCategory::IO),
    (1 << 9,  "RECLAIM",     "Page is being reclaimed", FlagCategory::Memory),
    (1 << 10, "BUDDY",       "Page is free buddy page", FlagCategory::Allocation),
    (1 << 11, "MMAP",        "Page is memory mapped", FlagCategory::Usage),
    (1 << 12, "ANON",        "Page is anonymous", FlagCategory::Usage),
    (1 << 13, "SWAPCACHE",   "Page is in swap cache", FlagCategory::Memory),
    (1 << 14, "SWAPBACKED",  "Page is swap backed", FlagCategory::Memory),
    (1 << 15, "COMPOUND_HEAD", "Compound page head", FlagCategory::Structure),
    (1 << 16, "COMPOUND_TAIL", "Compound page tail", FlagCategory::Structure),
    (1 << 17, "HUGE",        "Huge page", FlagCategory::Structure),
    (1 << 18, "UNEVICTABLE", "Page is unevictable", FlagCategory::Memory),
    (1 << 19, "HWPOISON",    "Hardware poisoned page", FlagCategory::Error),
    (1 << 20, "NOPAGE",      "No page frame exists", FlagCategory::State),
    (1 << 21, "KSM",         "KSM page", FlagCategory::Special),
    (1 << 22, "THP",         "Transparent huge page", FlagCategory::Structure),
    ...
    (1 << 32, "RESERVED",    "Reserved page (common in early memory)", FlagCategory::State),
];
...
impl KPageFlagsReader {
    ...
    fn get_total_pages(&mut self) -> Result<u64, Box<dyn std::error::Error>> {
        // Try to get file size, but handle cases where it might not work
        match self.file.seek(SeekFrom::End(0)) {
            Ok(file_size) => {
                // Reset position
                self.file.seek(SeekFrom::Start(0))?;
                Ok(file_size / 8) // Each entry is 8 bytes
            }
            Err(_) => {
                // If we can't get file size, return a reasonable default
                // This can happen with some virtual filesystems
                Ok(1048576) // Default to 1M pages (4GB of memory)
            }
        }
    }
...
fn visualize_flags_grid(pages: &[PageInfo], width: usize) {
    ...
    println!("  {} = State flags (LOCKED, DIRTY, etc.)", "S".color(colored::Color::Blue));
    println!("  {} = Memory mgmt (LRU, ACTIVE, etc.)", "M".color(colored::Color::Green));
    println!("  {} = Usage tracking (REFERENCED, ANON, etc.)", "U".color(colored::Color::Yellow));
    println!("  {} = Allocation (BUDDY, SLAB)", "A".color(colored::Color::Cyan));
    println!("  {} = I/O related (WRITEBACK)", "I".color(colored::Color::Magenta));
    println!("  {} = Structure (HUGE, THP, etc.)", "T".color(colored::Color::Red));
    println!("  {} = Special (KSM, ZERO_PAGE, etc.)", "P".color(colored::Color::White));
    println!("  {} = Error flags (ERROR, HWPOISON)", "E".color(colored::Color::BrightRed));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let count: u64 = if let Some(count_str) = matches.get_one::<String>("count") {
        if count_str == "all" {
            ...
            let mut reader_temp = KPageFlagsReader::new()?;
            let total_pages = reader_temp.get_total_pages()?;
            println!("Detected {} total pages in system", total_pages.to_string().cyan());
            total_pages.saturating_sub(start_pfn)
        } else {
            count_str.parse()?
        }
    } else {
    ...
    let grid_width: usize = matches.get_one::<String>("width")
        .unwrap()
        .parse()?;
    let output_limit: usize = matches.get_one::<String>("limit")
        .unwrap()
        .parse()?;
    ...
    if count > output_limit as u64 && !summary_only {
        println!("{}", format!("Note: Individual page output limited to first {} pages", output_limit).yellow());
    }
    ...
    if count > 10000 {
        println!("{}", "Reading page flags... (this may take a moment for large datasets)".yellow());
    }
    ...
    if !summary_only {
        ...
        let pages_to_show = if pages.len() > output_limit {
            println!("{}", format!("Showing first {} of {} pages:", output_limit, pages.len()).yellow());
            &pages[..output_limit]
        } else {
            &pages
        };
        ...
        if pages.len() > output_limit {
            println!("{}", format!("... and {} more pages (use --summary to see all statistics)", 
                pages.len() - output_limit).dimmed());
        }
    }
